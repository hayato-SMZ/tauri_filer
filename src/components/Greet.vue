...
const dirlist = () => {
  invoke("rootdir", { path: path.value != "" ? path.value : "/" })
    .then((res: unknown) => {
      ...
      list.value = list.value.concat(res_item.filter((item: any) => !item.is_file) as any[]);
  ...
  <div>
    ...
    <v-card>
      <v-list density="compact" lines="one">
        ...
        <v-list-item  v-for="item in dir_list" :key="item" @click="update" :prepend-icon="item.is_dir? 'mdi-folder':'mdi-file'">
